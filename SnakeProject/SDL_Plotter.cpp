...
bool SDL_Plotter::getQuit(){
    ...
    while( SDL_PollEvent( &event ) != 0 )
    {
        if(event.type == SDL_TEXTINPUT){
            ...
            if(currentKeyStates[SDL_SCANCODE_P]) key_queue.push(PAUSE);
            if(currentKeyStates[SDL_SCANCODE_U]) key_queue.push(UNPAUSE);
            if(currentKeyStates[SDL_SCANCODE_Q]) key_queue.push(Q);
            if(currentKeyStates[SDL_SCANCODE_1]) key_queue.push(ONE);
            if(currentKeyStates[SDL_SCANCODE_2]) key_queue.push(TWO);
            if(currentKeyStates[SDL_SCANCODE_3]) key_queue.push(THREE);
        }
